...
pub fn normalize_value(
    ...
    if column.data_type == DataType::Int {
        return value.as_i64().map(BindValue::Integer).ok_or_else(|| {
            ApiError::Validation(format!("Field {} must be an integer", column.display_name))
        });
    }
    if column.data_type == DataType::Float {
        return value.as_f64().map(BindValue::Float).ok_or_else(|| {
            ApiError::Validation(format!("Field {} must be a float", column.display_name))
        });
    }
